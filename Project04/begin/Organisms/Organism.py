...
class Organism(ABC):
	...
	def consequences(self, atackingOrganism):
		...
		if self.power > atackingOrganism.power:
			...
			print(atackingOrganism.__class__.__name__ + " died of an attack of a " + self.__class__.__name__ + str(atackingOrganism.position))
		else:
			result.append(Action(ActionEnum.A_REMOVE, Position(xPosition=-1, yPosition=-1), 0, self))
			print(self.__class__.__name__ + " died of an attack of a " + atackingOrganism.__class__.__name__ + str(self.position))
		return result
